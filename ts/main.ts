...
function calc(): void {
    ...
    if (falta < 0) return escrever("Parece que alguma nota é maior do que deveria.", COLORS.Error);
    if (falta > 100) return escrever("Infelizmente você já foi reprovado nessa disciplina.", COLORS.Reproved);
    
    var fst = ((falta + 0.5) / 10).toFixed(1);

    if (falta == 0) return escrever("Você já foi aprovado nessa disciplina!", COLORS.Approved);
