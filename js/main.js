...
var nota3_e = document.getElementById("nota3");
...
function calc() {
    ...
    if (n3 > 100)
        return escrever("Parece que a terceira nota é maior do que deveria.", COLORS.Error);
    ...
    if (soma >= 600)
        return escrever(`Parabéns! Você foi aprovado nessa diciplina! Sua nota final foi ${final}.`, COLORS.Approved);
    else {
        var falta = parseInt(((600 - 3 * (n1 + n2)) / 4 + 0.5).toString());
        var falta_str = getDecimal(falta, 1);
        if (falta > 100)
            return escrever(`Infelizmente, você foi reprovado nessa disciplina. Você precisava de ${falta_str} nesse último trimestre.`, COLORS.Reproved);
...
function getDecimal(nota, casas) {
